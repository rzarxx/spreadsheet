...
function FormContent() {
  ...
  const editRow = searchParams?.get("row");
  ...
  const fetchColumns = useCallback(async () => {
    ...
    setFetchError("");
    try {
      const url = editRow ? `/api/form/${projectId}?row=${editRow}` : `/api/form/${projectId}`;
      ...
      const initial = {};
      cols.forEach((col) => { 
        initial[col] = data.editData?.[col] || ""; 
      });
      setFormData(initial);
    } catch (err) {
      setFetchError(err.message || "Gagal memuat form.");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus({ type: "", message: "" });
    try {
      const res = await fetch(`/api/form/${projectId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ fields: formData, row: editRow }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setStatus({ type: "success", message: editRow ? "Data berhasil diperbarui!" : "Data berhasil disimpan!" });
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", height: "60px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <div style={{
                ...
                {editRow ? "Mode Edit Data" : "Formulir Aktif"}
              </div>
            </div>
          </div>
        </div>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: "80px", paddingBottom: "40px", padding: "80px 20px 40px" }}>
        ...
        <div style={{ maxWidth: "600px", textAlign: "center", position: "relative", zIndex: 1, marginBottom: "28px", animation: "fadeInUp 0.5s ease forwards" }}>
          <div style={{
            ...
            {editRow ? "Edit Data Spreadsheet" : "Sistem Input Otomatis"}
          </div>
          <h1 style={{ fontSize: "clamp(1.75rem, 4vw, 2.8rem)", fontWeight: 900, color: "#1e293b", margin: "0 0 12px", lineHeight: 1.2, letterSpacing: "-0.03em" }}>
            {editRow ? "Perbarui Data " : "Input Data Langsung ke "}
              ...
              <div>
                ...
                <div style={{ background: "#f8fafc", borderRadius: "14px", padding: "18px 20px", border: "1px solid #e2e8f0" }}>
                  ...
                  <ol style={{ margin: "0 0 14px", paddingLeft: "18px", color: "#64748b", fontSize: "13px", lineHeight: 2 }}>
                    <li>Pastikan URL Form valid (terdapat ID Project).</li>
                    <li>Pemilik Project harus terautentikasi dan memberikan izin Google Drive & Sheets.</li>
                    <li>Pastikan file Spreadsheet tidak dihapus dari Google Drive.</li>
                  </ol>
                ...
                <form id="dynamic-form" onSubmit={handleSubmit}>
                  ...
                  >
                    {loading ? (
                      ...
                      <><i className="fa-solid fa-paper-plane" /> {editRow ? "Perbarui Data" : "Kirim ke Spreadsheet"}</>
                    )}
                  </button>
                </form>
              </div>
...
export default function FormPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f1f5f9" }}><i className="fa-solid fa-spinner fa-spin" style={{ color: "#7c3aed", fontSize: "32px" }} /></div>}>
      <FormContent />
    </Suspense>
  );
}
